...
# CONFIGURATION
MODEL_PATH = os.path.join(os.path.dirname(__file__), "yolo11n-seg.onnx") 
WIDTH = 640
HEIGHT = 480
FPS = 30
INPUT_SIZE = 640
...
def main():
    print("=" * 60)
    print("YOLO Segmentation + RealSense - Jetson Orin (OpenCV)")
    print("=" * 60)
    
    # Charger le modèle ONNX avec OpenCV DNN
    ...
    print("[INFO] Backend: CPU")
    
    # Ouvrir la caméra RealSense via OpenCV
    print(f"\n[INFO] Ouverture caméra (index {CAMERA_INDEX})...")
    ...
    if not cap.isOpened():
        ...
        print("Listez les caméras avec: ls /dev/video*")
        return
    ...
    try:
        while True:
            ...
            # Inférence YOLO avec OpenCV DNN
            ...
            cv2.putText(annotated, f"Detections: {len(boxes)}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            cv2.imshow("YOLO + RealSense - Jetson", annotated)
            frame_count += 1
            cv2.waitKey(1)
